...
import './SaveHours.css';

const SaveHours = ({ onBack }) => {
    ...
    const handleSaveHours = async () => {
        ...
        addLog(`Starting sync from ${fromDate} to ${toDate}`);
        ...
        for (let i = 0; i < dates.length; i++) {
            ...
            const date = dates[i];
            setCurrentStep(`Syncing hours for ${date}...`);
            addLog(`Requesting hour data for ${date}...`);

            try {
                // The API expects from/to and the specific date to process
                const response = await fetch(`./api/sync_desktime_range.php?from=${fromDate}&to=${toDate}&date=${date}`);
                ...
                if (!response.ok) {
                    ...
                    try {
                        ...
                        if (errorText.includes('Maximum execution time')) {
                            errorMessage = "Server timeout (took too long)";
                        } else {
                ...
                if (result.success) {
                    successCount++;
                    addLog(`Successfully synced hours for ${date}`, 'success');
                } else {
        ...
        if (stopRequested.current) {
            ...
            setStatus({ type: 'success', message: `Successfully synced hours for ${totalSteps} days!` });
        } else {
    ...
    return (
        <div className="save-hours-container">
            <button className="back-btn" onClick={onBack}>← Back to Dashboard</button>
            
            <div className="save-hours-card">
                <header className="card-header">
                    <h2>Save Hours to Database</h2>
                    <p>Select a date range to fetch and store DeskTime records locally in the database.</p>
                </header>
                ...
                <div className="action-section">
                    <div style={{ display: 'flex', gap: '10px', width: '100%' }}>
                        <button
                            ...
                            style={{ flex: 1 }}
                        >
                            {loading ? 'Processing...' : 'Save Hours to Database'}
                        </button>
                        {loading && (
                            <button
                                className="stop-btn"
