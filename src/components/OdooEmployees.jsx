...
function OdooEmployees() {
  ...
  const fetchEmployees = async (page = 1, columns = null, search = '') => {
    ...
    try {
      ...
      const response = await fetch(`/api/get_odoo_employees.php?page=${page}&limit=${pagination.pageSize}${columnsParam}${searchParam}${sortParam}`)
  ...
  const exportToCSV = async () => {
    try {
      ...
      const response = await fetch(`/api/get_odoo_employees.php?page=1&limit=10000${columnsParam}${searchParam}${sortParam}`)
      const result = await response.json()
      ...
      link.setAttribute('download', `odoo_employees_export_${new Date().toISOString().slice(0, 10)}.csv`)
